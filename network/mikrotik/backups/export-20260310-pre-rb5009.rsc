# mar/10/2026 16:56:38 by RouterOS 6.49.19
...
add bridge=bridge client-to-client-forwarding=yes comment="VLAN 40 - IoT" \
...
add address=10.0.40.185 mac-address=A4:77:33:F8:98:6E server=dhcp-iot
...
/system scheduler
add comment="LIFX WiFi monitoring - temporary" interval=1m name=\
    lifx-capsman-monitor on-event=lifx-capsman-monitor policy=read,test \
    start-date=feb/21/2026 start-time=23:25:25
/system script
add dont-require-permissions=no name=lifx-capsman-monitor owner=admin policy=\
    read,test source=":local c 0; :foreach i in=[/caps-man registration-table \
    find] do={ :local mac [/caps-man registration-table get \$i mac-address]; \
    :local p [:pick \$mac 0 8]; :if (\$p = \"D0:73:D5\") do={ :local sig [/cap\
    s-man registration-table get \$i rx-signal]; :local up [/caps-man registra\
    tion-table get \$i uptime]; :local iface [/caps-man registration-table get\
    \_\$i interface]; :log info (\"LIFX-MON: mac=\$mac sig=\$sig up=\$up if=\$\
    iface\"); :set c (\$c + 1) } }; :log info (\"LIFX-MON: total=\$c bulbs\")"
/tool mac-server
